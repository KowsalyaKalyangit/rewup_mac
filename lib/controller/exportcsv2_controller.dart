...
class ExportDetailsCSV2Controller extends GetxController {
  ...
  Map map = {};
  Future getexportdetailscsv2Controller({fromdate, todate}) async {
    isexportcsvLoading(true);
    try {
      _listData.clear();
      var data = await ExportcsvService2().getExportcsvService2(fromdate: fromdate,todate: todate);
      if (data != null) {
        _listData.add(data);
  ...
  //pdf
  Future getexportdetailspdf2Controller({fromdate, todate}) async {
    isexportcsvLoading(true);
    try {
      _pdf.clear();
      var data = await ExportcsvService2().getExportpdfService2(fromdate: fromdate,todate: todate);
      if (data != null) {
        _pdf.add(data);
